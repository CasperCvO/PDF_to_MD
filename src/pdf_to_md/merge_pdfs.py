...
Usage
-----
    python -m pdf_to_md.merge_pdfs [source_dir] [-o output.pdf]

If *source_dir* is omitted, ``pdf_to_merge`` (in the current working directory)
is used. If *-o/--output* is omitted, ``merged.pdf`` is written to the current
working directory.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help=f"Directory containing PDFs to merge (default: {DEFAULT_SOURCE_DIR_NAME} in current directory).",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    source_dir = (
        ...
        if args.source_dir
        else (Path.cwd() / DEFAULT_SOURCE_DIR_NAME).resolve()
    )
